...
(function() {
    ...
    gs.info('Current script length: ' + script.length);
    
    // Check if already exists
    if (script.indexOf('deleteSession: function()') !== -1) {
        gs.info('⚠️  deleteSession method already exists!');
        return;
    }
    ...
    // Find insertion point - before HELPER METHODS or before submitVote
    ...
    if (insertPosition === -1) {
        ...
        gs.error('Looking for HELPER METHODS or submitVote');
        return;
    }
    
    gs.info('✅ Found insertion point at position: ' + insertPosition);
    ...
    gs.info('New script length: ' + newScript.length);
    ...
    gs.info('');
    gs.info('=== SCRIPT INCLUDE UPDATED ===');
    gs.info('✅ deleteSession method added successfully!');
    gs.info('New mod_count: ' + si.sys_mod_count);
    gs.info('');
    gs.info('Next: Run STEP 2 to update the UI Page');
